...
<div class='login'>
    ...
    <script>
        $('#dl').click(function(){
            ...
            var pdw=$('input[name=pwd]').val();
            //$.post('http://api.fictions.com/login',{user_name:'user_name',user_pwd:'user_pwd',back:a},function(){})
            $.ajax({
                url:'http://api.fictions.com/login?callback=',
                data:{user_name:name,user_pwd:pdw,'_token':'{{csrf_field()}}'},
                dataType:'jsonp',
                type:"post",
                ...
                success:function (res) {
                    ...
                    if(res.status==200){
                        $.post('login_do',{name:res.data.user_name,id:res.data.user_id},function(msg){
                            if(msg==1){
                                alert('登陆成功');
                                location.href="/"
                            }else{
                    ...
                    }else if(res.status=='00002'){
                        alert('账号密码不能为空')
                    }else if(res.status=='00001'){
                        alert('账号密码有误')
                    }
                }
            });
